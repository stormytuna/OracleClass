...
public class OraclePlayer : ModPlayer
{
    ...
    /// <summary>
    ///     This goes through your inventory and reduces the soul capacity of each weapon with that type. Call this whenever you reduce an items soul capacity
    ///     <para />
    ...
    public void ReduceSoulOnAllWeapons(Item itemToIgnore) {
		foreach (Item item in Player.inventory) {
			if (item.type == itemToIgnore.type && item != itemToIgnore) {
				OracleWeapon oracleWeapon = item.GetOracleWeapon();
	...
	public override void OnEnterWorld() {
		foreach (Item item in Player.inventory) {
			if (item.TryGetOracleWeapon(out OracleWeapon ow)) {
